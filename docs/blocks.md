...
新しいブロックを足すときは、**この表もふくめて 6 か所**に書きます。
手順は [`maintenance.md`](maintenance.md) の「ブロックを 1 つ足す」にまとめてあります。
どこか 1 つ忘れると `node tools\check-blocks.js` が名指しで止めるので、
**足したら、まずそれを走らせてください**（ビルドは要りません）。

とくに `ui/js/sim.js`（プレビュー）と `shiori/src/interp.cpp`（本番）は同じ規則を
二重に書いているので、**片方だけ直すと静かにズレます**。
`shiori/test/parity/ghost.json` に「毎回おなじ結果になる形」で足しておくと、
...
**リンクの URL・音のファイル名・選択肢の見出しと行き先**に入れた文字からは、
`[` `]` と改行（行き先はさらに `,`）を落としてから書き出します。
ここに「言われたこと」や「たずねた答え」のような**外から来た文字**をはめ込んでも、
`]` でタグを閉じて別の命令を続けることはできません。
`さくらスクリプト □ をそのまま出す`（`raw`）だけは、名前のとおり**素通し**です。
外から来た文字をそのまま入れないでください。
...
（SSP が固まらないための安全装置です）。
1 回に出すさくらスクリプトは 32000 バイトまでで、変数 1 つも 32000 バイトまでです。
こえたぶんは出ません（途中で切るのではなく、そのかたまりを丸ごと落とします）。
